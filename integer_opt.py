...
@LastEditTime: 2020-05-08 16:02:00
@LastEditors: Ye Han
@Description:
...
def integer_opt(number_of_pet, number_of_pcs, pet_power_demand, block_cdq, pet_state, pet_lat, pet_lon, number_of_region, pet_region, pet_soc, pet_pick_up_probability, block_plq, plq_arrival_rate, delay_aware_arrival_rate, block_delay_aware, utility_function, pet_remaining_power, V, per_service_fee, pev_arrival_rate, cdq_service_rate, pcs_cost):
    ...
    pet_pick_up_ini = pet.apply(lambda x: np.random.choice([0, 1], p=[
        1-x['pick_up_probability'], x['pick_up_probability']]) if((x['state'] == 0) & (x['soc'] > 10)) else 0, axis=1).values.reshape(1, number_of_pet)
    ...
    pet_pick_up_region = pet_region_matrix @ pet_pick_up.T
    ...
    # Arrival rate max.
    constraints_6_right = np.full((number_of_pcs, 1), 250)
    constraints = [state_test + pet_non_recommended.T >= constraints_2_right,
                   cv.multiply(soc_test, x) == constraints_4_right, pet_recommended <= constraints_5_right]
    problem = cv.Problem(cv.Minimize(objects), constraints)
    problem.solve(solver=cv.CPLEX)
    return x.value, pet_pick_up.value.reshape(number_of_pet, 1), pet_pick_up_region.value, section_plq.value, section_delay_aware.value, section_cdq.value
...
if __name__ == '__main__':
    ...
    integer_opt(number_of_pet, number_of_pcs, pet_power_demand, block_cdq, pet_state, pet_lat, pet_lon, number_of_region, pet_region, pet_soc,
                pet_pick_up_probability, block_plq, plq_arrival_rate, delay_aware_arrival_rate, block_delay_aware, utility_function, pet_remaining_power, V, per_service_fee)

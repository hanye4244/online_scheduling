...
for V in V_list:
    for worst_case_delay_guarantee in worst_case_delay_guarantee_list:
        ...
        block_cdq = np.full((number_of_pcs, 1), 0)
        block_plq = np.full((number_of_region, 1), 0)
        block_delay_aware = np.full((number_of_region, 1), 0)
        waiting_demand = np.zeros((number_of_pcs, number_of_pet))
        charging_test = np.zeros((number_of_pcs, number_of_pet))
        charging_state = np.zeros((number_of_pcs, number_of_pet))
        # Time slot iteration.
        for t in range(504):
            pet_pick_up = np.zeros((number_of_pet, 1))
            pet_put_down = np.zeros((number_of_pet, 1))
            ...
            charging_time = pet_power_demand / (0.15 * 10)
        ...
        print('block_cdq_list =', np.mean(block_cdq_list))
        print('block_plq_list=', np.mean(block_plq_list))

...
def pet_location_time_slot(pet_state, pet_lat, pet_lon, number_of_pet, action, pcs_lat, pcs_lon, pet_trigger_recommended, pet_completed):
    ...
    pet['pet_completed'] = list(pet_completed)
    pet['pet_trigger_recommended'] = pet_trigger_recommended
    pet['lat'] = pet.apply(lambda x: ((40 - 39.9) * np.random.random() + 39.9)
                           if ((x['pet_trigger_recommended'] != 1) & (x['state'] != 2) & (x['pet_completed'] != 1)) else x['lat'], axis=1)
    pet['lon'] = pet.apply(lambda x: ((116.4 - 116.3) * np.random.random() + 116.3)
                           if ((x['pet_trigger_recommended'] != 1) & (x['state'] != 2) & (x['pet_completed'] != 1)) else x['lon'], axis=1)
